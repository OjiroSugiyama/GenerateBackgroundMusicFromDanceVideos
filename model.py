...
import labanFutureValue
from excitement import Pkl_Reader
...
class Model():
    ...
    def __init__(self, line_height, block_width, line_block_height):
        ...
        # *AIST++の骨格情報が格納されているcsvFile
        # self.file = "C:\\Users\gorim\Desktop\Motion-Aware-Sequencer\output-20211016T031203Z-001\output\gHO_sFM_cAll_d19_mHO4_ch05.csv"
        # self.excitement = Excitement(self.file)
        # self.motion_excitement_array = self.excitement.motion_excitement_array
        # print(len(self.motion_excitement_array))


    def initializationExcitement(self,pixel_array):
        """盛り上がりの初期化"""
        print("pixcel_array:"+str(len(pixel_array)))
        print("self.block_width:"+str(self.block_width))
    ...
    def chengeExcitement(self, pixcel_array):
        ...
        print("change:"+str(self.excitement_array))
    ...
    def useHMM(self, excitement_array):
        """HMMを使用する"""
        # print("excitement_array:"+str(excitement_array))
    ...
    def analysisMovie(self,movie_file_path):
        ...
        # ファイル名の処理
        basename = os.path.basename(movie_file_path)
        basename = os.path.splitext(basename)[0]
        split_name = basename.split('_')
        file_name = split_name[0]+"_"+split_name[1]+"_cAll_"+split_name[3]+"_"+split_name[4]+"_"+split_name[5]+".pkl"
        # file_name = split_name[0]+"_"+split_name[1]+"_cAll_"+split_name[3]+"_"+split_name[4]+"_"+split_name[5]+".csv"
        print(file_name)
        # excitement = Pkl_Reader("C:\\Users\gorim\Desktop\Motion-Aware-Sequencer\motions\\"+ file_name)
        excitement = labanFutureValue.LabanFutureValue("C:\\Users\gorim\Desktop\Motion-Aware-Sequencer\keypoints3d\\"+ file_name)
        
        if self.movie_analysing:
            self.movie_analysing = False 
            # *excitement.pyで計算した盛り上がり度の格納
            # nodes, frame = excitement.pkl_read()
            # excitement.calcMotionExcitement(nodes, frame)
            # excitement.randomMotionExcitement(nodes, frame)
            # self.motion_excitement_array = excitement.motion_excitement_array
            # print(len(self.motion_excitement_array))
            # self.calcMovieExcitement(self.movie, self.direction_cnt_array)

            excitement.OnInterval()
            self.motion_excitement_array = excitement.excitement
